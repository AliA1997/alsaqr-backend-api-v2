...
namespace AlSaqr.API.Controllers.SocialMedia
{
    ...
    public class ListsController : AuthorizedControllerBase
    {
        ...
        )
        {
            ...
            if (user == null)
                return Unauthorized("User must be logged in to get lists.");
            ...
            if (
                noSearchTerm
                && _socialMediaCacheService.CheckIfInitialListsCanBeRetrieved(currentPage, userId)
            )
        ...
        )
        {
            ...
            try
            {
                var newlyCreatedListId = await _listRepository.CreateList(
                    _supabase,
                    userId,
                    data,
                    ct
                ...
                {
                    await _listItemRepository.AddUsersToList(
                        _supabase,
                        newlyCreatedListId,
                        data.UsersAdded.ToList(),
                        ct
                    );
                }
                ...
                {
                    await _listItemRepository.AddPostsToList(
                        _supabase,
                        newlyCreatedListId,
                        data.PostsAdded.ToList(),
                        ct
                    );
                }
        ...
        )
        {
            ...
            if (
                _socialMediaCacheService.CheckIfInitialListItemForListCanBeRetrieved(
                    currentPage,
                    userId,
                    listId
                )
                ...
                return Ok(
                    _socialMediaCacheService.GetInitialListItemsForList(userId, listId, currentPage)
                );

            var result = await _listItemRepository.GetListItems(
                _supabase,
                userId,
                listId,
                currentPage,
        ...
        )
        {
            ...
            try
            {
                await _listItemRepository.SaveItemToList(
                    _supabase,
                    userId,
                    listId,
                    data,
            ...
            catch (Exception err)
            {
                ...
                return StatusCode(
                    500,
                    new { message = "Saved item to list error!", success = false }
                );
            }
        }
        ...
        public async Task<IActionResult> DeleteSavedFromList(Guid listId, Guid listItemId)
        {
            var authError = ValidateAccessToken();
            if (authError != null)
                return authError;
